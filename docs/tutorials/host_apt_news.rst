...
.. list-table::
   ...
   * - ``you@yourcomputer:~$``
     - Your computer
   * - ``ubuntu@apt-news-tutorial:~$``
     - Multipass VM named ``apt-news-tutorial``
   * - ``root@apt-news-server:~#``
     - LXD container named ``apt-news-server``
   * - ``root@client-noble:~#``
     - LXD container named ``client-noble``
   * - ``root@client-jammy:~#``
...
.. code-block:: console
   ...
   you@yourcomputer:~$ multipass launch noble --disk 10G --name apt-news-tutorial

Access the VM:

.. code-block:: console

   you@yourcomputer:~$ multipass shell apt-news-tutorial
   ubuntu@apt-news-tutorial:~$
...
.. code-block:: console
   ...
   ubuntu@apt-news-tutorial:~$ lxd init --minimal


Launch three containers inside the VM with the following commands:

.. code-block:: console

   ubuntu@apt-news-tutorial:~$ lxc launch ubuntu-daily:noble apt-news-server
   ubuntu@apt-news-tutorial:~$ lxc launch ubuntu-daily:noble client-noble
   ubuntu@apt-news-tutorial:~$ lxc launch ubuntu-daily:jammy client-jammy
...
accomplish this with by installing and configuring `nginx`_ to serve a
...
.. code-block:: console
   ...
   ubuntu@apt-news-tutorial:~$ lxc shell apt-news-server
   root@apt-news-server:~#
...
.. code-block:: console
   ...
   root@apt-news-server:~# apt install nginx
...
.. code-block:: console
   ...
   ubuntu@apt-news-tutorial:~$ lxc exec client-noble -- pro config set apt_news_url=http://apt-news-server/aptnews.json
   ubuntu@apt-news-tutorial:~$ lxc exec client-jammy -- pro config set apt_news_url=http://apt-news-server/aptnews.json
...
.. code-block:: console
   ...
   ubuntu@apt-news-tutorial:~$ lxc shell client-jammy
...
And in the Noble container we expect to see the other message:

.. code-block:: console

   ubuntu@apt-news-tutorial:~$ lxc shell client-noble
   root@client-noble:~# pro refresh messages
   root@client-noble:~# apt upgrade
   Reading package lists... Done
   Building dependency tree... Done
   Reading state information... Done
   Calculating upgrade... Done
...
.. code-block:: console
   ...
   ubuntu@apt-news-tutorial:~$ exit
   you@yourcomputer:~$ multipass delete --purge apt-news-tutorial
