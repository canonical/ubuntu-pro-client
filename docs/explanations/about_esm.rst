...
updates provided by Canonical to your Ubuntu system. Setting the preference
...
repositories are going to be preferred over these as well. That means if a
particular package happens to have two different versions, one coming from the
PPA and the other from ESM, the ESM version will be chosen over, even if the
PPA version is higher.

If you don't want this behavior, that is, you want the the PPA version to be
