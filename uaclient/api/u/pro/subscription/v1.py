...
def _subscription(cfg: UAConfig) -> SubscriptionResult:
    ...
    expires = (
        machine_token_file.contract_expiry_datetime
        if contractInfo.get("effectiveTo")
        else contractInfo.get("effectiveTo", None)
    )
    ...
    if support:
        ...
        if supportLevel:
            tech_support_level = supportLevel
    ...
    # Subscription Result
    activity_id = (
        machine_token_file.activity_id
        if machine_token_file.activity_id
        else ""
    )
    return SubscriptionResult(
        contract=contract,
