...
class TestFixPlan:
    ...
    @pytest.mark.skip(reason="bug fix reverted; pure refactor only")
    @mock.patch(M_PATH + "merge_usn_released_binary_package_versions")
    @mock.patch(M_PATH + "get_cve_affected_source_packages_status")
    @mock.patch(M_PATH + "_get_cve_data")
    @mock.patch(M_PATH + "query_installed_source_pkg_versions")
    @mock.patch(M_PATH + "_check_cve_fixed_by_livepatch")
    def test_fix_plan_for_cve_when_released_source_has_no_binary_metadata(
        self,
        m_check_cve_fixed_by_livepatch,
        m_query_installed_pkgs,
        m_get_cve_data,
        ...
        """Regression test for released sources without comparable binaries.

        If release metadata marks a source package as released but does not
        include metadata for any installed binary package, the plan should not
        conclude "already fixed". The plan should consider the package "still
        vulnerable".
        """
        m_check_cve_fixed_by_livepatch.return_value = (None, None)
        m_query_installed_pkgs.return_value = {
        ...
        result = fix_plan_cve(issue_id="cve-1234-1235", cfg=mock.MagicMock())

        assert str(FixStatus.SYSTEM_STILL_VULNERABLE) == result.current_status
        assert not any(
            isinstance(step, FixPlanNoOpAlreadyFixedStep)
    ...
    @mock.patch(M_PATH + "query_installed_source_pkg_versions")
    @mock.patch(M_PATH + "_check_cve_fixed_by_livepatch")
    def test_fix_plan_for_cve_when_all_sources_have_comparable_metadata(
        self,
        m_check_cve_fixed_by_livepatch,
        m_query_installed_pkgs,
        m_get_cve_data,
