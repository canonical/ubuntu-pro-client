...
class TestCloudInstanceFactory:
    ...
    def test_raise_error_when_not_aws(self, m_get_cloud_type):
        ...
        with pytest.raises(exceptions.UserFacingError) as excinfo:
            cloud_instance_factory()
        error_msg = status.MESSAGE_UNSUPPORTED_UBUNTU_PRO_CLOUD_TYPE.format(
            cloud_type="nonaws"
        )
        assert error_msg == str(excinfo.value)

    @mock.patch("uaclient.clouds.aws.UAProAWSInstance")
    def test_raise_error_when_not_viable_aws_ubuntu_pro(
        self, m_aws_instance, m_get_cloud_type
    ):
        """Raise error when the AWS instance is not viable Ubuntu Pro."""
        ...
        with pytest.raises(exceptions.UserFacingError) as excinfo:
            cloud_instance_factory()
        error_msg = status.MESSAGE_UNSUPPORTED_UBUNTU_PRO
        assert error_msg == str(excinfo.value)
