...
class TestProxyPromptHelpers:
    @pytest.mark.parametrize(
        "args, kwargs, expected_prompt, prompt_ret, expected_ret",
        [
            (
                ("service", "protocol"),
                {"curr_proxy": None, "new_proxy": None, "assume_yes": False},
                False,
                False,
                None,
            ),
    ...
    def test_prompt_for_proxy_change(
        self,
        ...
        m_prompt_for_confirmation.return_value = prompt_ret
        ret = util.prompt_for_proxy_change(*args, **kwargs)
            ...
            assert [
                mock.call(
                    status.MESSAGE_PROXY_OVERRIDE_PROMPT.format(
                        service=args[0],
                        protocol=args[1],
                        curr_value=kwargs["curr_proxy"],
                        new_value=kwargs["new_proxy"],
                    ),
                    assume_yes=kwargs["assume_yes"],
                )
            ] == m_prompt_for_confirmation.call_args_list

    @mock.patch(
        "uaclient.util.prompt_for_proxy_change", side_effect=["ret1", "ret2"]
    )
    def test_prompt_for_proxy_changes(self, m_prompt_for_proxy_change):
        ret = util.prompt_for_proxy_changes(
            "service",
            curr_http_proxy="curr_http",
            curr_https_proxy="curr_https",
            new_http_proxy="new_http",
            new_https_proxy="new_https",
            assume_yes=True,
        )
        ...
        assert [
            ...
            mock.call(
                "service",
                "https",
                curr_proxy="curr_https",
                new_proxy="new_https",
                assume_yes=True,
            ),
        ] == m_prompt_for_proxy_change.call_args_list
