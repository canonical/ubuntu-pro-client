...
def action_config_show(args, *, cfg, **kwargs):
    ...
    if args.key:  # limit reporting config to a single config key
        ...
        print(
            "{key} {value}".format(
                key=args.key, value=getattr(uaconfig, args.key, None)
            )
        )
        return 0
    ...
    for key in config.UA_CONFIGURABLE_KEYS:
        print(row_tmpl.format(key=key, value=getattr(uaconfig, key, None)))

    if (
        uaconfig.global_apt_http_proxy or uaconfig.global_apt_https_proxy
    ) and (uaconfig.ua_apt_http_proxy or uaconfig.ua_apt_https_proxy):
