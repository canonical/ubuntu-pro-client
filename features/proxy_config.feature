...
Feature: Proxy configuration
    ...
    Scenario Outline: Attach command when proxy is configured
        ...
        When I create the file `/etc/apt/apt.conf.d/50-testproxy` with the following
        """
        ...
        And I configure uaclient `apt_http` proxy to use `proxy` machine
        And I configure uaclient `apt_https` proxy to use `proxy` machine
        When I run `ua refresh config` with sudo, and provide the following stdin
        """
        n
        y
    ...
    Scenario Outline: Attach command when proxy is configured
        ...
        And I run `ua refresh config` with sudo, and provide the following stdin
        """
        n
        y
